...
<div style="max-width:900px;margin:32px auto;padding:0 16px">
  ...
  <div style="background:#1a56db;border-radius:14px;padding:24px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
    <div style="display:flex;align-items:center;gap:16px">
      ...
      <div>
        <p style="font-size:18px;font-weight:700;color:#fff;margin:0">{{ auth()->user()->name }}</p>
        <p style="font-size:13px;color:rgba(255,255,255,0.75);margin:3px 0 0">{{ auth()->user()->email }}</p>
        <p style="font-size:12px;color:rgba(255,255,255,0.6);margin:2px 0 0">Miembro desde {{ auth()->user()->created_at->format('M Y') }}</p>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
      <div style="text-align:center">
        <p style="font-size:28px;font-weight:800;color:#fff;margin:0">{{ $bids->count() }}</p>
          ...
          <tbody>
            ...
            <tr style="border-top:1px solid #f3f4f6">
              <td style="padding:12px 16px">
                <a href="{{ route('auctions.show', $bid->auction_id) }}" style="text-decoration:none">
                  <p style="font-size:13px;font-weight:600;color:#111;margin:0">{{ Str::limit($bid->auction->title ?? '—', 30) }}</p>
              ...
              <td class="hide-mobile" style="padding:12px 16px">
                <p style="font-size:13px;color:#374151;margin:0">€{{ number_format($bid->auction->current_price ?? 0,0,',','.') }}</p>
              </td>
              <td style="padding:12px 16px">
              ...
              <td style="padding:12px 16px">
                ...
                @elseif($bid->auction)
                  <a href="{{ route('auctions.show', $bid->auction_id) }}" style="font-size:12px;color:#1a56db;text-decoration:none;font-weight:600;white-space:nowrap">Ver →</a>
                @endif
              </td>
            </tr>
            @endforeach
