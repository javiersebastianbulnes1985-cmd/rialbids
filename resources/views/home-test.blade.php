@extends('layouts.app')
@section('title','RialBids — Subastas Online')
@section('content')
...
<div style="position:relative;width:100%;height:380px;overflow:hidden;background:#1a5f7a;">
  ...
  <div style="position:relative;z-index:2;max-width:1280px;margin:0 auto;padding:0 24px;height:100%;display:flex;flex-direction:column;justify-content:center;">
    ...
    @endif
    @if($banner->link)
      <a href="{{ $banner->link }}" style="display:inline-block;padding:12px 28px;background:#fff;color:#111;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;width:fit-content;">
        {{ $banner->link_texto }} →
      </a>
    @endif
  </div>
...
<div style="max-width:1280px;margin:32px auto;padding:0 24px;">
        ...
        @foreach($filtered as $auction)
          ...
          <a href="{{ route('auctions.show', $auction->id) }}"
            ...
            <div style="position:relative;aspect-ratio:1/1;background:#f8f8f8;overflow:hidden;">
              @if($img)
                <img src="{{ $img }}" alt="{{ $auction->title }}" style="width:100%;height:100%;object-fit:contain;padding:8px;" loading="lazy">
              @endif
            </div>
  ...
  @else
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
        <a href="/?categoria=relojes" style="border-radius:10px;height:150px;position:relative;overflow:hidden;text-decoration:none;display:block;">
          ...
          <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,20,50,0.85) 0%,rgba(26,58,140,0.15) 60%);display:flex;flex-direction:column;align-items:flex-start;justify-content:flex-end;padding:16px;">
            <span style="font-size:17px;font-weight:700;color:#fff;">{{ __('Relojes') }}</span>
            <span style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:2px;">8 {{ __('lotes activos') }}</span>
          </div>
        </a>
        <a href="/?categoria=joyas" style="border-radius:10px;height:150px;position:relative;overflow:hidden;text-decoration:none;display:block;">
          ...
          <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,20,50,0.85) 0%,rgba(26,58,140,0.15) 60%);display:flex;flex-direction:column;align-items:flex-start;justify-content:flex-end;padding:16px;">
            <span style="font-size:17px;font-weight:700;color:#fff;">{{ __('Joyería') }}</span>
            <span style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:2px;">7 {{ __('lotes activos') }}</span>
          </div>
        </a>
        ...
        <a href="/?categoria=coleccionismo" style="border-radius:10px;height:150px;position:relative;overflow:hidden;text-decoration:none;display:block;">
          ...
          <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,20,50,0.85) 0%,rgba(26,58,140,0.25) 60%);display:flex;align-items:flex-end;padding:16px;">
            <span style="font-size:17px;font-weight:700;color:#fff;">{{ __('Coleccionismo') }}<br><small style="font-size:11px;font-weight:400;color:rgba(255,255,255,0.7);">{{ __('Próximamente') }}</small></span>
          </div>
        </a>
        <a href="/?categoria=otros" style="border-radius:10px;height:150px;position:relative;overflow:hidden;text-decoration:none;display:block;">
          ...
          <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,20,50,0.85) 0%,rgba(26,58,140,0.25) 60%);display:flex;align-items:flex-end;padding:16px;">
            <span style="font-size:17px;font-weight:700;color:#fff;">{{ __('Cámaras') }}<br><small style="font-size:11px;font-weight:400;color:rgba(255,255,255,0.7);">{{ __('Próximamente') }}</small></span>
          </div>
        </a>
      </div>
    </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;">
        <div style="background:#0f2744;border-radius:10px;overflow:hidden;">
          ...
          <div style="padding:18px 22px 22px;">
            ...
            <span style="font-size:13px;color:rgba(255,255,255,0.55);">8 {{ __('lotes activos') }}</span>
            <a href="/auctions?categoria=relojes" style="display:inline-block;margin-top:8px;border:1px solid rgba(255,255,255,0.3);color:#fff;font-size:13px;padding:9px 20px;border-radius:4px;text-decoration:none;">{{ __('Explorar →') }}</a>
          </div>
        </div>
        <div style="background:#0f2744;border-radius:10px;overflow:hidden;">
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px;padding:8px 8px 0;">
          ...
          <div style="padding:18px 22px 22px;">
            ...
            <span style="font-size:13px;color:rgba(255,255,255,0.55);">7 {{ __('lotes activos') }}</span>
            <a href="/auctions?categoria=joyeria" style="display:inline-block;margin-top:8px;border:1px solid rgba(255,255,255,0.3);color:#fff;font-size:13px;padding:9px 20px;border-radius:4px;text-decoration:none;">{{ __('Explorar →') }}</a>
          </div>
        </div>
        </div>
      </div>
    ...
    @endphp
    <div style="margin-bottom:40px;">
      <h2 style="font-size:18px;font-weight:700;color:#111;margin-bottom:20px;">{{ __('Colecciones') }}</h2>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;">
        @foreach($colecciones as $col)
          @php $citems = $activeAuctions->whereIn('id',$col['ids']); @endphp
          @if($citems->count() > 0)
            <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:16px;">
              <h4 style="font-size:14px;font-weight:700;color:#111827;margin-bottom:10px;">{{ $col['titulo'] }} <span style="font-size:11px;font-weight:400;color:#9ca3af;">({{ $citems->count() }})</span></h4>
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;">
                @foreach($citems->take(3) as $it)
                  @php $cimg = !empty($it->image_path) ? (str_starts_with($it->image_path,'http') ? $it->image_path : asset('storage/'.$it->image_path)) : null; @endphp
                  <div style="aspect-ratio:1/1;background:#f8f8f8;border-radius:4px;overflow:hidden;">
                    @if($cimg)<img src="{{ $cimg }}" style="width:100%;height:100%;object-fit:cover;">@endif
                  </div>
                @endforeach
              </div>
            </div>
  ...
  <div style="background:#0f2744;border-radius:12px;padding:32px;margin-top:-16px;margin-bottom:0;text-align:center;">
    <h2 style="font-size:22px;font-weight:700;color:#fff;margin:0 0 8px;">{{ __('Tu primer lote, sin comisión') }}</h2>
    <p style="font-size:14px;color:rgba(255,255,255,0.7);margin:0 0 20px;">{{ __('Registrate como vendedor y publicá tu primera subasta sin pagar comisión de venta.') }}</p>
    <a href="/seller-request" style="display:inline-block;background:#fff;color:#0f2744;padding:10px 26px;border-radius:6px;font-size:13px;font-weight:700;text-decoration:none;">{{ __('Empezar ahora') }}</a>
  </div>
