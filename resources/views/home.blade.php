...
<div style="position:relative;width:100%;height:420px;overflow:hidden;background:#1a56db;">
  @if($banner->imagen_path)
    <img src="{{ asset('storage/'.$banner->imagen_path) }}"
...
<div style="background:#1a56db;padding:22px 24px;">
  <div style="max-width:1280px;margin:0 auto;display:flex;justify-content:space-between;align-items:center;">
    <div>
      ...
      <p style="font-size:13px;color:rgba(255,255,255,0.8);margin:5px 0 0;">{{ $auctions->count() }} lotes activos disponibles ahora</p>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;">
      ...
      <div style="padding:8px 0;">
        <a href="{{ route('home') }}"
          ...
          <span>Todos</span>
          <span style="background:#f3f4f6;color:#6b7280;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;">{{ $auctions->count() }}</span>
        </a>
        @foreach($categorias as $slug => $nombre)
          @php $count = $auctions->where('lot_category', $slug)->count(); @endphp
          @if($count > 0)
          <a href="{{ route('home', ['categoria' => $slug]) }}"
            ...
            <span>{{ $nombre }}</span>
            <span style="background:#f3f4f6;color:#6b7280;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;">{{ $count }}</span>
          </a>
          @endif
        @endforeach
      </div>
  ...
  <div style="flex:1;min-width:0;">
    ...
    @if($filtered->isEmpty())
      <div style="text-align:center;padding:60px 0;color:#9ca3af;">
        ...
        <a href="{{ route('home') }}" style="color:#1a56db;font-size:13px;">Ver todas →</a>
      </div>
    @else
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
        @foreach($filtered as $auction)
          ...
          <a href="{{ route('auctions.show', $auction->id) }}"
             style="display:block;background:#fff;border:1px solid {{ $urgent?'#fca5a5':'#e5e7eb' }};border-radius:10px;overflow:hidden;text-decoration:none;position:relative;transition:box-shadow .2s,transform .2s;"
            ...
            <div style="padding:12px 14px;">
              ...
              <div style="display:flex;justify-content:space-between;align-items:flex-end;">
                <div>
                  ...
                  <p style="font-size:18px;font-weight:700;color:{{ $tb>0?'#16a34a':'#111827' }};">€{{ number_format($cp,0,',','.') }}</p>
                ...
                <div style="text-align:right;">
                  ...
                  @elseif($urgent)
                    <p style="font-size:13px;font-weight:700;color:#ef4444;">
                      @if($d>0){{ $d }}d {{ str_pad($hh,2,'0',STR_PAD_LEFT) }}h
                      @elseif($hh>0){{ str_pad($hh,2,'0',STR_PAD_LEFT) }}:{{ str_pad($mm2,2,'0',STR_PAD_LEFT) }}h
                      @else{{ str_pad($mm2,2,'0',STR_PAD_LEFT) }}:{{ str_pad($ss,2,'0',STR_PAD_LEFT) }}m
                      @endif
                    </p>
                    <p style="font-size:10px;color:#ef4444;font-weight:600;">⚡ Cierra pronto</p>
                  @else
                    <p style="font-size:13px;font-weight:600;color:#374151;">{{ $d }}d {{ str_pad($hh,2,'0',STR_PAD_LEFT) }}h</p>
                  @endif
                </div>
              </div>
            </div>
...
{{-- FINALIZA PRONTO --}}
@if(isset($finaliza_pronto) && $finaliza_pronto->isNotEmpty())
<div style="max-width:1280px;margin:0 auto 40px;padding:0 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
    ...
    <a href="{{ route('home') }}" style="font-size:13px;color:#1a56db;text-decoration:none;font-weight:600;">Mostrar todo →</a>
  </div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
    @foreach($finaliza_pronto as $auction)
      @php
        $img = null;
        if(!empty($auction->image_path)) $img = asset('storage/'.$auction->image_path);
        elseif(!empty($auction->image_path_2)) $img = asset('storage/'.$auction->image_path_2);
      ...
      <a href="{{ route('auctions.show', $auction->id) }}"
        ...
        <div style="padding:12px 14px;">
          <h3 style="font-size:13px;font-weight:600;color:#111827;line-height:1.4;margin-bottom:6px;">{{ Str::limit($auction->title, 50) }}</h3>
          <p style="font-size:12px;color:#ef4444;font-weight:600;margin-bottom:4px;">
            @if($d>0){{ $d }}d {{ str_pad($hh,2,'0',STR_PAD_LEFT) }}h
            @elseif($hh>0){{ str_pad($hh,2,'0',STR_PAD_LEFT) }}h {{ str_pad($mm2,2,'0',STR_PAD_LEFT) }}m
            @else{{ str_pad($mm2,2,'0',STR_PAD_LEFT) }}m restantes
            @endif
          </p>
          <p style="font-size:11px;color:#9ca3af;">{{ $tb }} {{ $tb==1?'puja':'pujas' }}</p>
          <p style="font-size:16px;font-weight:700;color:{{ $tb>0?'#16a34a':'#111827' }};">€{{ number_format($cp,0,',','.') }}</p>
        </div>
      </a>
    @endforeach
  </div>
...
@if(isset($recientes) && $recientes->isNotEmpty())
<div style="max-width:1280px;margin:0 auto 40px;padding:0 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
    @foreach($recientes as $auction)
      ...
      <a href="{{ route('auctions.show', $auction->id) }}"
        ...
        <div style="padding:12px 14px;">
          <h3 style="font-size:13px;font-weight:600;color:#111827;line-height:1.4;margin-bottom:6px;">{{ Str::limit($auction->title, 50) }}</h3>
          <p style="font-size:11px;color:#9ca3af;margin-bottom:2px;">{{ $tb }} {{ $tb==1?'puja':'pujas' }}</p>
          <p style="font-size:16px;font-weight:700;color:{{ $tb>0?'#16a34a':'#111827' }};">€{{ number_format($cp,0,',','.') }}</p>
        </div>
      </a>
    @endforeach
...
@if(isset($finalizadas) && $finalizadas->isNotEmpty())
<div style="max-width:1280px;margin:0 auto 40px;padding:0 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
    <div style="display:flex;align-items:center;gap:12px;">
      <h2 style="font-size:18px;font-weight:700;color:#111827;margin:0;">Subastas Finalizadas</h2>
      <span style="background:#f3f4f6;color:#6b7280;font-size:12px;font-weight:600;padding:3px 10px;border-radius:20px;">{{ $finalizadas->count() }}</span>
    </div>
    <a href="{{ route('auctions.finalizadas') }}" style="font-size:13px;color:#1a56db;text-decoration:none;font-weight:600;">Ver todas →</a>
  </div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
    @foreach($finalizadas as $auction)
      ...
      <a href="{{ route('auctions.show', $auction->id) }}"
        ...
        <div style="padding:12px 14px;">
          <h3 style="font-size:13px;font-weight:600;color:#374151;line-height:1.4;margin-bottom:8px;">{{ Str::limit($auction->title, 50) }}</h3>
          <div style="display:flex;justify-content:space-between;align-items:flex-end;">
            <div>
              <p style="font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:1px;">Precio final</p>
              <p style="font-size:16px;font-weight:700;color:#6b7280;">€{{ number_format($cp,0,',','.') }}</p>
            </div>
            @if($ef)
            <p style="font-size:11px;color:#9ca3af;">{{ \Carbon\Carbon::parse($ef)->format('d/m/Y') }}</p>
            @endif
          </div>
        </div>
      </a>
