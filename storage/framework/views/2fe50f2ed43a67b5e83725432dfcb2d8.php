...
<div class="admin-wrap">
  <div class="sidebar">
    ...
    <div class="sb-section">
      ...
      <a href="<?php echo e(route('admin.pagos')); ?>" class="sb-link">🔴 Pagos y Disputas</a>
      <a href="<?php echo e(route('admin.automatizaciones')); ?>" class="sb-link">📧 Automatizaciones</a>
    </div>
    <div class="sb-section">
  ...
  <div class="main">
    ...
    <div class="stats">
      ...
      <div class="stat-card">
        ...
        <div class="stat-val"><?php echo e($auctions->sum('total_bids')); ?></div>
        ...
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($pimg): ?><img src="<?php echo e($pimg); ?>" class="lot-img"><?php else: ?><div class="lot-img" style="display:flex;align-items:center;justify-content:center;font-size:18px">📷</div><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
              <div>
                <div style="font-weight:600;color:#111827"><?php echo e(Str::limit($auc->title,40)); ?></div>
                <div style="font-size:11px;color:#9ca3af">#<?php echo e(str_pad($auc->id,4,'0',STR_PAD_LEFT)); ?></div>
              </div>
            </div>
          </td>
          <td>
            <div style="font-weight:600;font-size:12px"><?php echo e($auc->user->name ?? '—'); ?></div>
            <div style="font-size:11px;color:#9ca3af"><?php echo e($auc->user->email ?? ''); ?></div>
          </td>
          <td style="font-weight:700;color:#1a56db">€<?php echo e(number_format($auc->base_price,0,',','.')); ?></td>
          <td><?php echo e($auc->lot_category ?? '—'); ?></td>
          <td>
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
              <a href="<?php echo e(route('auctions.show',$auc->id)); ?>" class="btn btn-ghost" target="_blank">👁 Ver</a>
    ...
    <div class="card" id="lotes">
      <div class="card-header">
        ...
        <div class="filter-bar">
          <a href="<?php echo e(route('admin.index')); ?>" class="filter-btn <?php echo e(!request('estado') ? 'active' : ''); ?>">Todos (<?php echo e($auctions->count()); ?>)</a>
          <a href="<?php echo e(route('admin.index')); ?>?estado=active" class="filter-btn <?php echo e(request('estado')==='active' ? 'active' : ''); ?>">Activos (<?php echo e($auctions->where('status','active')->count()); ?>)</a>
          <a href="<?php echo e(route('admin.index')); ?>?estado=pending" class="filter-btn <?php echo e(request('estado')==='pending' ? 'active' : ''); ?>">Pendientes (<?php echo e($auctions->where('status','pending')->count()); ?>)</a>
          <a href="<?php echo e(route('admin.index')); ?>?estado=finished" class="filter-btn <?php echo e(request('estado')==='finished' ? 'active' : ''); ?>">Finalizados (<?php echo e($auctions->where('status','finished')->count()); ?>)</a>
        </div>
      </div>
      <table class="table">
        <thead><tr>
        ...
        ?>
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $filtrados; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $auc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $img = !empty($auc->image_path) ? (str_starts_with($auc->image_path,'http') ? $auc->image_path : asset('storage/'.$auc->image_path)) : null; ?>
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($img): ?><img src="<?php echo e($img); ?>" class="lot-img"><?php else: ?><div class="lot-img" style="display:flex;align-items:center;justify-content:center;font-size:18px">📷</div><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
              <div>
                <div style="font-weight:600;color:#111827;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?php echo e($auc->title); ?></div>
                <div style="font-size:11px;color:#9ca3af">#<?php echo e(str_pad($auc->id,4,'0',STR_PAD_LEFT)); ?></div>
              </div>
            </div>
          </td>
          <td>
            <div style="font-size:12px;font-weight:600"><?php echo e($auc->user->name ?? '—'); ?></div>
          </td>
          <td style="font-weight:700;color:#1a56db;white-space:nowrap">€<?php echo e(number_format($auc->current_price??$auc->base_price,0,',','.')); ?></td>
          <td style="text-align:center"><?php echo e($auc->total_bids ?? 0); ?></td>
          <td><span class="badge <?php echo e($badgeClass[$auc->status] ?? 'badge-pending'); ?>"><?php echo e($estados[$auc->status] ?? $auc->status); ?></span></td>
          <td style="font-size:12px;white-space:nowrap"><?php echo e($auc->end_time ? \Carbon\Carbon::parse($auc->end_time)->format('d/m/y H:i') : '—'); ?></td>
          <td>
